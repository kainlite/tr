...
export const Metamask = {
  mounted() {
    ...
    window.addEventListener(`phx:connect-metamask`, (e) => {
      // web3Provider.provider.request({ method: "eth_requestAccounts" }).then(
      //   (accounts) => {
      //     if (accounts.length > 0) {
      ...
      // );
      // this.pushEvent("connect-metamask", { connected: true });
    });

    // window.addEventListener("phx:coin-transaction", (event) => {
    // let contract = new ethers.Contract(
    //   event.detail.contract_address,
    //   metaCoinAbi,
    //   signer
    // );
